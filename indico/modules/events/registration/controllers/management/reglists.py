...
from indico.util.date_time import format_currency, now_utc, relativedelta, format_date
...
class RHRegistrationsExportPDFTable(RHRegistrationsExportBase):
    ...
    def _process(self):
        headers, rows = prepare_participant_list_data(reglist=self.registrations, display=self.export_config['regform_items'],
        ...
        html_source = render_template(template_path,
                                      event=self.event,
                                      headers=headers,
                                      rows=rows,
